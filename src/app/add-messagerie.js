...
files.forEach(f => {
  ...
  try {
    ...
    if (content.includes("naviguer('messagerie')")) {
      console.log('Deja present:', f);
      return;
    }
    // Trouver la ligne avec carte et ajouter apres la balise fermante </a>
    const idx = content.indexOf(searchText);
    if (idx === -1) {
      console.log('Carte non trouve:', f);
      return;
    }
    // Trouver la fin de la balise </a> apres carte
    const endIdx = content.indexOf('</a>', idx) + 4;
    content = content.slice(0, endIdx) + addText + content.slice(endIdx);
    fs.writeFileSync(fullPath, content, 'utf8');
    console.log('OK:', f);
  } catch(e) {
    console.log('Erreur:', f, e.message);
  }
});
console.log('Termine!');
